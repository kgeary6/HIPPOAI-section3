...
def get_predicted_volumes(pred):
    ...
    volume_ant = np.sum(pred==1) #anterior
    volume_post = np.sum(pred==2) #posterior
    total_volume = volume_ant + volume_post #total
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    header_font = ImageFont.truetype("src/assets/RobotoMono-Regular.ttf", size=40)
    main_font = ImageFont.truetype("src/assets/RobotoMono-Regular.ttf", size=20)
...
if __name__ == "__main__":
    ...
    # STAND OUT SUGGESTION: save_report_as_dcm has some suggestions if you want    to expand your  # knowledge of DICOM format
